...
//               src={slide.image}
//               alt={`Slide ${index + 1}`}
//               className={`w-full h-full ${index === 2 ? "object-contain mt-32" : "object-cover"}`}
//             />
//             {/* Captions */}
    ...
    <div className="relative w-full h-screen overflow-hidden" id="home">
      ...
      >
        {slides.map((slide, index) => (
          <div
            ...
            className="relative w-full h-screen flex-shrink-0 flex items-center justify-center"
          >
            {/* If it's an iframe slide, render iframe */}
            {slide.iframe ? (
              <iframe
      ...
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-3 z-20">
        {slides.map((_, index) => (
          <button
            key={index}
            className={`w-4 h-4 rounded-full ${currentIndex === index ? "bg-blue-500" : "bg-gray-300"}`}
            onClick={() => setCurrentIndex(index)}
          />
        ))}
      </div>
